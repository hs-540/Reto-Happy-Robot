...
- **Real actions**: built and wired end to end, behind an explicit switch. `backend/src/happyrobot.ts` has a real client that `POST`s `{ "prompt": ..., "missionId": ... }` to the HappyRobot mission hook (`HAPPYROBOT_WEBHOOK_URL`, with the hook's key as `x-api-key` when it is guarded), where `missionId` is the id of the action the agent decided — a call is traceable end to end. No phone number travels: the mission knows who it calls. The return path is the events API: when the voice agent hangs up, the hook posts a call-summary event to the worker and `backend/src/outcome-poll.ts` polls it (`EVENTS_POLL_MS`, default 10 s), matches each summary by mission and hands it to `agent.closeCall()`, where a refusal or a delay becomes a replanning trigger. **Real telephony is off by default**: `HAPPYROBOT_REAL_CALLS_ENABLED` is checked before anything else, so no leftover hook URL in an `.env` can make somebody's phone ring. With the switch off, `createHappyRobotClient()` picks the simulated client, which exercises that exact same chain with scripted answers.
...
real calls takes three variables together — the switch set to `true`,
`HAPPYROBOT_WEBHOOK_URL` pointing at the mission hook and `EVENTS_API_KEY` so
the closures can be polled back.
...
All variables go in `.env` (repo root, outside git). See [`.env.example`](.env.example): port, tick cadence and time scale, LLM provider (`LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`, `LLM_EMBEDDING_MODEL`), the real-telephony switch (`HAPPYROBOT_REAL_CALLS_ENABLED`) with the mission hook and call queue behind it, the events API that closes real calls, and Chroma settings.
