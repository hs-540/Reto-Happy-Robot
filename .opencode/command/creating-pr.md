---
description: Push the current branch and open a GitHub pull request, following this project's title/description conventions.
model: helmcode/glm5.3
---

Open a pull request for the current branch against `main`, using `gh`, kept consistent with the `commiting` and `branching-setup` commands.

## PR title format

`<type>: <message>`

- `type`: one of `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, `style`, `perf`, `ci`, `build` — same set used by the `commiting`/`branching-setup` commands, pick the one that best matches the overall change.
...
3. If there are uncommitted changes, warn the user and ask whether to commit them first (use the `commiting` command), stash, or continue anyway — do not assume.
4. Push the branch:
...
- Do not merge the PR — this command only opens it.
